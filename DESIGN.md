...
The structure of a review page — its sections and their order — is fixed in
[docs/review-page-standard.md](docs/review-page-standard.md) and generated by
`mailman review` and `mailman packet`. This file governs how it looks; that one
governs what it says and in what order. No agent hand-writes either.
...
The first thing a viewer should notice is the recommendation and the three
panels under it: what was broken, what we did, whether it is actually fixed.
Then what he has to answer, then what is still open, then the evidence — the
patch, the gate Mailman ran itself, and the agents' own accounts kept visibly
apart from it. Prose before code. An agent's report is the most
