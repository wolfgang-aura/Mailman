...
The repository starts with an intentionally narrow v0.1. It can create a private local run record, capture a command result with a timeout and redaction, enforce the workflow state machine, and report whether the local machine has the required tools. It does not yet invoke Codex or Claude, clone a target repository, or publish anything to GitHub.
...
This is foundation work, not a claim that the complete agent loop works. See [SOURCE_OF_TRUTH.md](SOURCE_OF_TRUTH.md) for verified environment facts and [ROADMAP.md](ROADMAP.md) for the next thin slice.
