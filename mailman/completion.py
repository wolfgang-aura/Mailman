...
def git_bytes(workspace: Path, *args: str) -> bytes:
    ...
    if result.returncode:
        raise ValueError(
            "Git inspection failed: " + result.stderr.decode("utf-8", "replace")
        )
...
def check_authorship(directory: Path, *, head: str | None = None) -> dict:
    ...
    if identity is None:
        raise ValueError(
            "identity-missing: configure `mailman identity` before preparing a submission"
        )
    ref = "HEAD"
    if head:
        ...
        if tip != current:
            raise ValueError(
                "head-mismatch: the filing branch differs from the inspected workspace HEAD"
            )
        if git_bytes(workspace, "status", "--porcelain").strip():
            raise ValueError(
                "uncommitted-candidate: commit the reviewed change before handoff"
            )
    commits = branch_commits(workspace, run.base_commit)
    if head and not commits:
        raise ValueError(
            "empty-branch: no candidate commits exist on the filing branch"
        )
    violations = author_violations(commits, identity)
    if violations:
        # The diagnostic names commits, never the private addresses.
        raise ValueError(
            "author-identity: correct author/committer identity on "
            + ", ".join(v["sha"] for v in violations)
        )
    return {
        "ok": True,
        "commits": len(commits),
        "head": git_bytes(workspace, "rev-parse", ref).decode().strip(),
    }
...
def finalize_review(directory: Path) -> dict:
    ...
    if run.status not in (
        ...
        raise ValueError(
            "engineering-incomplete: complete independent verification and review first"
        )
    decision = load_decision(directory)
    orchestration = read_object(directory / "orchestration.json")
    finals = [
        s
        for s in orchestration.get("steps", [])
        if s.get("name") == "verification:final" and s.get("ok")
    ]
    if not finals:
        raise ValueError(
            "final-verification-missing: no passing final verification exists"
        )
    evidence = finals[-1].get("data", {})
    expected = evidence.get("candidate_digest")
    if not expected:
        raise ValueError(
            "candidate-unbound: repeat verification and independent review with this Mailman version"
        )
    workspace = Path(evidence["workspace"])
    if candidate_digest(workspace, run.base_commit) != expected:
        raise ValueError(
            "candidate-changed: repeat verification and independent review for the edited candidate"
        )
    if run.status is RunStatus.ENGINEERING_COMPLETE:
        run.transition(
            RunStatus.READY_FOR_HUMAN_REVIEW,
            "decision validated against the verified candidate",
        )
    ...
    record = {
        ...
        "decision_digest": hashlib.sha256(
            (directory / "decision.json").read_bytes()
        ).hexdigest(),
